...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	svr4 = false;
	posix = false;
	if(!strcmp(argv_zero_string, "/usr/local/bin/sunmake")) {
		svr4 = false;
		posix = true;
	} else {
...
/* azv 16 Oct 95 */
		...
		if (exists(current_target) != file_doesnt_exist) {
			...
			if (current_target->stat.is_dir) {
				...
				(void) fprintf(stderr,
					       gettext("%s could not be removed: %s.\n"),
					       current_target->string_mb,
					       errmsg(errno));
			}
		}
...
/*
	...
	if ((ret = readlink("/usr/local/bin/sunmake", origin,
	    PATH_MAX - 1)) < 0)
	...
	origin[ret] = '\0';
*/
	return strdup(dirname("/usr/local/bin/sunmake"));
}
 ...
 */
	...
	if (!ignore_default_mk) {
		if (svr4) {
			MBSTOWCS(wcs_buffer, "/usr/local/share/sunmake/svr4.make.rules");
			default_makefile = GETNAME(wcs_buffer, FIND_LENGTH);
		} else {
			MBSTOWCS(wcs_buffer, "/usr/local/share/sunmake/make.rules");
			default_makefile = GETNAME(wcs_buffer, FIND_LENGTH);
		}
